...
public class Face_dect_info {
    ...
    public String the_whole(String file_path) throws Exception {
        ...
        System.out.println(httpRequests.detectionDetect(new PostParameters().setImg(imgbyte)));
...
//        for (int i = 0; i < arrayLength; i++) {
        ...
        people_add_face(name);
    }

    public void people_add_face(String name) {
     ...
     * 训练的结果可以通过/info/get_session查询。当训练完成时，返回值中将包含{"success": true}
     */
    public void train_identify_for_group(String name) {
        System.err.println("------------train_identify_for_group--------------");
        try {
            System.out.println(httpRequests.trainIdentify(new PostParameters().setGroupName(name)));
        } catch (FaceppParseException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
     ...
     * 先拍照识别，再检测
     */
    public String recognition_identify_face_inGroup(String name) {
        JSONObject result = null;
        try {
